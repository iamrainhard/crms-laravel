        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <a href="/users/create"
               class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                    class="fas fa-user-plus fa-sm text-white-50"></i> Register User</a>
        </div>

        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">List of All Registered Users</h6>
            </div>
            <div class="card-body">
                @if($users->isNotEmpty())
                                ...
                                <div class="col-sm-12">
                                    <table class="table table-bordered dataTable" id="dataTable" width="100%"
                                        ...
                                        @foreach($users as $user)
                                            <tr class="odd">
                                                <td class="sorting_1">{{$user->firstName}} {{$user->sirName}}</td>
                                                <td>{{$user->email}}</td>
                                                <td>{{$user->mobile}}</td>
                                                <td>{{$user->gender}}</td>
                                                <td>{{$user->role}}</td>
                                                <td>
                                                    <a href="users/{{ $user->id }}/edit" class="btn-circle btn-success"><i
                                                            class="fas fa-edit"></i></a>
                                                    <a href="/users/{{$user->id}}/delete" class="btn-circle btn-danger"><i
